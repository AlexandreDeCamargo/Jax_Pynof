...
def der_CJCKD5(n,ista,dn_dgamma,no1,ndoc,nalpha,nbeta,nv,nbf5,ndns,ncwo):
    ...
    # Interpair Electron correlation #
    # print('pynof dndgamma',dn_dgamma)
    Dcj12r = np.zeros((nbf5,nbf5,nv))
    Dck12r = np.zeros((nbf5,nbf5,nv))
    for k in prange(nv):
...
def calcoccg(gamma,J_MO,K_MO,H_core,p):
    ...
    grad = np.zeros((p.nv))
    grad_e_n = np.zeros((p.nv))
    n,dn_dgamma = ocupacion(gamma,p.no1,p.ndoc,p.nalpha,p.nv,p.nbf5,p.ndns,p.ncwo,p.HighSpin,p.occ_method)
    # print('n_pnof',n)
    # print('dn_dgamma_pnof',dn_dgamma)
    ...
    if(p.MSpin==0):
        ...
        diag = np.diag_indices(p.nbf5)
        Dck12r[diag] = 0
        # print('Dck12r_pnof',Dck12r)
        grad -= 2*np.einsum('ijk,ji->k',Dck12r,K_MO,optimize=True)
        # print(grad)
        # print('gamma',gamma)
        # grad_e_n += 2*H_core + np.diagonal(J_MO) + 2*np.einsum('ijk,ji->k', Dcj12r, J_MO,optimize=True)
        # grad_e_n -= 2*np.einsum('ijk,ji->k', Dck12r, K_MO,optimize=True)
        # grad_E_n = 2*H_core + np.diagonal(J_MO) + 2*np.einsum('ijk,ji->k', Dcj12r, J_MO,optimize=True)
        # -2*np.einsum('ijk,ji->k', Dck12r, K_MO,optimize=True)
        
        # print('grad_E_n',grad_E_n)
        # print('n',n)
        # print('db_dgamma',dn_dgamma)
        # print('grad_pynof',grad)
        # assert 0 
    elif(not p.MSpin==0):
